...
auto_format_version() {
    version=$1
    # Check if the version is already in the correct format (e.g., 15.0, 16.0)
    ...
    # If the version doesn't have a decimal, add ".0"
        ...
        echo "Invalid input. Please enter a valid Odoo version (e.g., 15.0 or 16.0)."
    fi
}

# Prompt the user to enter the Odoo version
echo "Enter odoo_version (eg: 15.0 or 16.0):"
read odoo_version
...
while true; do
read -p "Do you Want to install venv for odoo? (y/n) " yn5

read -p "Enter the Ubuntu version (eg.20.04 OR 22.04:" 
...
sudo apt-get install -y python3-dev python3-venv python3-wheel libxml2-dev libxslt1-dev zlib1g-dev libsasl2-dev libldap2-dev build-essential libssl-dev libffi-dev libmysqlclient-dev libjpeg-dev libpq-dev libjpeg8-dev liblcms2-dev libblas-dev libatlas-base-dev
...
case $yn5 in [yY] ) echo ok, odoo-venv installing.....;
sleep 3

cd /home/$username/$folder_name/
sudo python3 -m venv $odoo_version-venv
sudo chown -R $username: /home/$username/
cd odoo
